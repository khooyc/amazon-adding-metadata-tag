...
- Language, theme, and tutorial choices are stored only in the local Windows app profile.

A private, human-first Windows desktop app for preparing local Amazon listing images that contain photorealistic AI-generated people. It adds the exact keyword `contains-synthetic-performer` to XMP `dc:subject`, then re-reads that explicit field before reporting the local file as verified.
...
- Media stays on this PC; there are no network or cloud AI integrations.
...
- File deletion uses the Windows Recycle Bin only.
...
The packaged application bundles ExifTool 13.59 through the pinned `exiftool-vendored.exe` package.
