  ...
  async recoverInterruptedTransactions() {
    ...
    for (const transactionId of manifestIds) {
      ...
      try {
        ...
        const safetyBackup = this.store.getBackup(normalizePath(transaction.sourcePath), transaction.snapshotHash);
        const verifiedBackupPath = safetyBackup?.backupPath && path.resolve(safetyBackup.backupPath);
        const backupRelative = verifiedBackupPath ? path.relative(path.resolve(this.backupRoot), verifiedBackupPath) : null;
        if (!verifiedBackupPath || !backupRelative || backupRelative.startsWith('..') || path.isAbsolute(backupRelative)
          || await sha256File(verifiedBackupPath) !== transaction.snapshotHash) {
          throw new Error('Recovery transaction does not match a verified safety backup.');
        }
